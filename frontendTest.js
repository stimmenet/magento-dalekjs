...
/*    'Page title is correct': function (test) {
        test
                .open(config.url)
                .assert.title().is('Madison Island', 'It has title')
                .done();
    },
    ...
    'Women Category exists': function (test) {
        test
                .open(config.url)
                .query('#nav ol li.level0.nav-1.first.parent a')
                .assert.visible()
                .assert.text().is("WOMEN")
                .click('#nav > ol > li.first')
                .waitForElement('body')
                .assert.title().is('Women', 'It has title')
                .end()
                .done();
